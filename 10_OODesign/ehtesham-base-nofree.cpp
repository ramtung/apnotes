...
class EmployeeOrder {
public:
  EmployeeOrder(Employee* employee, MenuItem* item, int quantity=1) {
  ...
  MenuItem* get_menu_item() const { return item_; }
  Employee* get_employee() const { return employee_; } 
...
public:
  CateringOrderLine(MenuItem* menu_item, int quantity = 1) {
    menu_item_ = menu_item;
    quantity_ = quantity;
  }
  MenuItem* get_menu_item() const { return menu_item_; }
  int get_quantity() const { return quantity_; }
...
public:
  ...
  void AddEmployeeOrder(const EmployeeOrder* emp_order) {
    for (auto line : orderLines_)
      if (line->get_menu_item() == emp_order->get_menu_item()) {
        line->IncreaseQuantity(emp_order->get_quantity());
        return;
      }
    orderLines_.push_back(new CateringOrderLine(emp_order->get_menu_item()));
  }
private:
  vector<CateringOrderLine*> orderLines_;
};
...
public:
  ...
  void TakeEmployeeOrder(Employee* emp, string item_title) {
    employee_orders_.push_back(new EmployeeOrder(emp, FindItemByTitle(item_title)));
  }
  ...
  void HandleDelivery(int delivery_cost) const {
    ...
    for (const auto& emp_order : employee_orders_) {
      Employee* emp = emp_order->get_employee();
...
private:
  MenuItem* FindItemByTitle(string item_title) {
  ...
  vector<EmployeeOrder*> employee_orders_;
};
...
public:
  ...
  void TakeEmployeeOrder(string emp_id, string catering_name, string item_title) {
    Catering* cat = FindCateringByName(catering_name);
    Employee* emp = FindEmployeeById(emp_id);
  ...
  string BedehiReport() {
    ostringstream os;
    for (const Employee* emp : employees_) {
...
private:
  ...
  Catering* FindCateringByName(string cat_name) {
